...
class Bdd(torch.utils.data.Dataset):
    ...
    BDD video loader. Construct the BDD video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=100):
        """
        Construct the BDD video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for BDD".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing BDD {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load BDD split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing bdd dataloader (size: {} skip_rows {}) from {} ".format(
