...
class Carla(torch.utils.data.Dataset):
    """
    (Copied from kinetics.py)
    Carla video loader. Construct the Carla video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=100):
        """
        Construct the Carla video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Carla".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing Carla {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load Carla split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing carla dataloader (size: {} skip_rows {}) from {} ".format(
